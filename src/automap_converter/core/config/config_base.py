...
try:
    ...
    class QObject:  # type: ignore[no-redef]
        """Small Qt-free base class for headless conversion configurations."""
    ...
    def pyqtSignal():  # type: ignore[no-redef]
        return _NoOpSignal()

from automap_converter.core.config.yaml_support import register_interval_yaml

T = TypeVar("T")


register_interval_yaml()


class Attribute(Generic[T], QObject):#把一个属性的值、默认值、显示名称、描述、单位、选项和验证函数封装在一个类中，并提供了方法来设置、验证和重置属性的值，以及订阅属性值变化的信号。
