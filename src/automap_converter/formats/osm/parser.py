...
from automap_converter.core.config.osm_config import osm_config as config
...
from automap_converter.conversion.osm_to_commonroad.converter_modules.graph_operations.restrictions import (
    Restriction,
)
from automap_converter.conversion.osm_to_commonroad.converter_modules.graph_operations.road_graph._graph import (
    Graph,
)
from automap_converter.conversion.osm_to_commonroad.converter_modules.graph_operations.road_graph._graph_edge import (
    GraphEdge,
)
from automap_converter.conversion.osm_to_commonroad.converter_modules.graph_operations.road_graph._graph_node import (
    GraphNode,
)
from automap_converter.conversion.osm_to_commonroad.converter_modules.graph_operations.road_graph._graph_traffic_light import (
    GraphTrafficLight,
)
from automap_converter.conversion.osm_to_commonroad.converter_modules.graph_operations.road_graph._graph_traffic_sign import (
    GraphTrafficSign,
)
from automap_converter.conversion.osm_to_commonroad.converter_modules.graph_operations.road_graph._sublayered_graph import (
    SublayeredGraph,
)
from automap_converter.formats.osm import info_deduction as i_d
from automap_converter.conversion.osm_to_commonroad.converter_modules.utility import idgenerator
from automap_converter.conversion.osm_to_commonroad.converter_modules.utility.custom_types import (
    Road_info,
)
...
def get_ways(
    accepted_highways: List[str], rejected_tags: Dict[str, str], root
) -> OrderedSet[ElTree.Element]:
    """
...
def parse_file(
    ...
    traffic_rules = get_traffic_rules(
        road_nodes, ways, config.TRAFFIC_SIGN_KEYS, config.TRAFFIC_SIGN_VALUES
    )
...
def get_graph_traffic_signs(
    ...
    for traffic_sign in traffic_signs:
        ...
        if node_id.startswith("road"):
            ...
            graph_traffic_sign = GraphTrafficSign(
...
def get_graph_traffic_lights(
    nodes: Dict[int, GraphNode], traffic_lights: List[Dict]
) -> List[GraphTrafficLight]:
    """
...
def map_restrictions(
    ...
    for from_id, restrictions in restrictions.items():
        if from_id in edges:
            ...
            logging.warning(
                "unknown id '{}' for restriction element. skipping restriction".format(from_id)
            )
...
def get_crossing_points(
    ...
    for point_id, point in main_cross_points.items():
        if point_id in sub_cross_points and not close_to_intersection(
            point_id, comb_graph, main_graph
        ):
...
def create_graph(file_path: str) -> Graph:
    ...
    if config.EXTRACT_SUBLAYER:
        if _value_set(config.ACCEPTED_HIGHWAYS_MAINLAYER) & _value_set(
