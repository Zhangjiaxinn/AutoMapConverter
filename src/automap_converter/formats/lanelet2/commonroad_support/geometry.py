...
def _to_same_dim(a: np.ndarray, b: np.ndarray):
    import numpy as np
    a = np.asarray(a, dtype=float).reshape(-1)
    b = np.asarray(b, dtype=float).reshape(-1)
    if a.shape[0] == 2 and b.shape[0] == 3:
        a = np.append(a, 0.0)
    elif a.shape[0] == 3 and b.shape[0] == 2:
        b = np.append(b, 0.0)
    return a, b
...
def distance(point: np.ndarray, polyline: Iterable[np.ndarray]) -> float:
    ...
    for linepoint in polyline:
        p, lp = _to_same_dim(point, linepoint)
        distances.append(np.linalg.norm(p - lp))
...
def point_to_line_distance(point: np.ndarray, line_start: np.ndarray, line_end: np.ndarray) -> float:
    """
    Calculates the perpendicular distance of a point to a line defined by two points.
    Robust to mixing 2D (x,y) and 3D (x,y,z): pads 2D vectors with z=0.
    """
    import numpy as np

    point = np.asarray(point, dtype=float).reshape(-1)
    line_start = np.asarray(line_start, dtype=float).reshape(-1)
    line_end = np.asarray(line_end, dtype=float).reshape(-1)

    # Harmonize dimensions: pad 2D to 3D with z=0
    dims = max(point.shape[0], line_start.shape[0], line_end.shape[0])
    if dims not in (2, 3):
        raise ValueError(f"Unsupported point dimension: {dims}")

    def pad(v: np.ndarray, d: int) -> np.ndarray:
        if v.shape[0] == d:
            return v
        if v.shape[0] == 2 and d == 3:
            return np.append(v, 0.0)
        raise ValueError(f"Cannot pad vector of shape {v.shape} to dim {d}")

    point = pad(point, dims)
    line_start = pad(line_start, dims)
    line_end = pad(line_end, dims)

    denom = np.linalg.norm(line_end - line_start)
    if denom == 0:
        # Degenerate line: distance to a point
        return float(np.linalg.norm(point - line_start))

    return float(np.linalg.norm(np.cross(line_end - line_start, line_start - point)) / denom)
