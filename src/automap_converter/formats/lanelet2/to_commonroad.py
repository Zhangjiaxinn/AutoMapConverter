...
from automap_converter.formats.lanelet2.commonroad_support.conversion_lanelet import ConversionLanelet
from automap_converter.formats.lanelet2.commonroad_support.conversion_lanelet_network import (
    ConversionLaneletNetwork,
)
from automap_converter.formats.lanelet2.commonroad_support.geometry import (
    distance as point_to_polyline_distance,
)
from automap_converter.formats.lanelet2.commonroad_support.geometry import point_to_line_distance
from automap_converter.formats.lanelet2.commonroad_support.utils import generate_unique_id
...
def _wrong_left_right_boundary_side(
    ...
    """Return whether the left/right boundaries are swapped.

    This converter-local geometric check replaces the former dependency on the
    unrelated verification package.  For each centerline segment, a positive
    2-D cross product denotes the geometrical left side of travel.
    """

    if len(center_vertices) < 2 or len(left_vertices) != len(right_vertices):
        return False
    tangent = np.diff(center_vertices[:, :2], axis=0)
    left_offset = left_vertices[:-1, :2] - center_vertices[:-1, :2]
    right_offset = right_vertices[:-1, :2] - center_vertices[:-1, :2]
    left_cross = tangent[:, 0] * left_offset[:, 1] - tangent[:, 1] * left_offset[:, 0]
    right_cross = tangent[:, 0] * right_offset[:, 1] - tangent[:, 1] * right_offset[:, 0]
    valid = np.logical_and(np.isfinite(left_cross), np.isfinite(right_cross))
    if not np.any(valid):
        return False
    return float(np.median(left_cross[valid])) < float(np.median(right_cross[valid]))
...
class Lanelet2CRConverter:
    ...
    def __init__(
        ...
        crs_from = CRS(LANELET2_DEFAULT_CRS)
    ...
    def _convert_way_to_vertices(self, way: Way) -> np.ndarray:
        ...
        """
        # Always use 3D vertices for robustness
        vertices = np.empty((len(way.nodes), 3), dtype=float)
        ...
        for i, node_id in enumerate(way.nodes):
            ...
            y -= self.origin_utm[1]
            try:
                z = float(nd.ele) if nd.ele is not None else 0.0
            except Exception:
                z = 0.0
            vertices[i] = [x, y, z]
            #vertices[i] = [x, y, float(nd.ele)] if nd.ele != "0.0" else [x, y]#有误
