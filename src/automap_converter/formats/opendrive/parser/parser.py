...
from automap_converter.formats.opendrive.parser.utils import (
    clean_projection_string,
    generate_unique_id,
)
from automap_converter.formats.opendrive.parser.elements.junction import (
...
from automap_converter.formats.opendrive.parser.elements.roadElevationProfile import (
...
from automap_converter.formats.opendrive.parser.elements.roadLateralProfile import (
...
from automap_converter.formats.opendrive.parser.elements.roadLink import (
...
from automap_converter.formats.opendrive.parser.elements.roadObject import (
...
from automap_converter.formats.opendrive.parser.elements.roadtype import (
    RoadType,
)
from automap_converter.formats.opendrive.parser.elements.roadtype import (
...
def parse_opendrive_road_signal(new_road: Road, road_signal: etree.ElementTree):
    ...
    for user_data in road_signal.findall("userData"):
        code = user_data.get("code")
        value = user_data.get("value", "")
        if not code:
            continue
        new_signal.user_data.setdefault(code, []).append(value)
    if (
...
def parse_opendrive_road_object(new_road: Road, obj: etree.ElementTree):
    ...
    if len(outlines) > 0:
        for outline in outlines:
            ...
            for corner_local in outline.findall("cornerLocal"):
                ...
                corner.id = corner_local.get("id")
                corner.height = corner_local.get("height")
                corners.append(corner)
            for corner_road in outline.findall("cornerRoad"):
                corner = ObjectOutlineCorner()
                corner.coordinate_type = "road"
                corner.s = float(corner_road.get("s"))
                corner.t = float(corner_road.get("t"))
                corner.dz = float(corner_road.get("dz", "0"))
                corner.id = corner_road.get("id")
                corner.height = corner_road.get("height")
                corners.append(corner)
            if corners:
                parsed_outlines.append(corners)
    ...
    try:
        ...
        road_object.outlines = parsed_outlines
        road_object.outline = parsed_outlines[0] if parsed_outlines else []
        for user_data in obj.findall("userData"):
            code = user_data.get("code")
            value = user_data.get("value", "")
            if not code:
                continue
            road_object.user_data.setdefault(code, []).append(value)
    except KeyError:
        logging.error(
