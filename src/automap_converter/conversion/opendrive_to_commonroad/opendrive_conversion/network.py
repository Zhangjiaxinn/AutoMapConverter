...
from automap_converter.core.common_file_reader_writer import project_lanelet
...
from automap_converter.conversion.opendrive_to_commonroad.support.conversion_lanelet_network import (
...
from automap_converter.conversion.opendrive_to_commonroad.opendrive_conversion.converter import (
    OpenDriveConverter,
)
from automap_converter.conversion.opendrive_to_commonroad.opendrive_conversion.plane_elements.crosswalks import (
    get_crosswalks,
)
from automap_converter.conversion.opendrive_to_commonroad.opendrive_conversion.plane_elements.geo_reference import (
    get_geo_reference,
)
from automap_converter.conversion.opendrive_to_commonroad.opendrive_conversion.plane_elements.plane_group import (
    ParametricLaneGroup,
)
from automap_converter.conversion.opendrive_to_commonroad.opendrive_conversion.plane_elements.traffic_signals import (
    assign_traffic_signals_to_road,
    get_traffic_signal_references,
)
from automap_converter.conversion.opendrive_to_commonroad.opendrive_conversion.utils import (
    encode_road_section_lane_width_id,
)
from automap_converter.formats.opendrive.parser.elements.opendrive import (
    OpenDrive,
)
...
def convert_to_base_lanelet_network(lanelet_network: ConversionLaneletNetwork) -> LaneletNetwork:
    ...
    for area in lanelet_network.areas:
        network.add_area(copy.deepcopy(area), set())
    for la in lanelet_network.lanelets:
        ...
        base_lanelet.opendrive_road_id = getattr(la, "opendrive_road_id", None)
        base_lanelet.opendrive_lane_id = getattr(la, "opendrive_lane_id", None)
        base_lanelet.opendrive_lane_section_id = getattr(
            la, "opendrive_lane_section_id", None
        )
        base_lanelet.opendrive_parametric_lane_id = getattr(
            la, "opendrive_parametric_lane_id", None
        )
        base_lanelet.opendrive_lane_locations = set(
            getattr(la, "opendrive_lane_locations", set())
        )
        base_lanelet.opendrive_lane_locations_detailed = set(
            getattr(la, "opendrive_lane_locations_detailed", set())
        )
        network.add_lanelet(base_lanelet)
    return network
...
class Network:
    ...
    def load_opendrive(self, opendrive: OpenDrive):
        ...
        for road in opendrive.roads:
            ...
            # Get crosswalks
            self._crosswalks.extend(get_crosswalks(road, self._config))
    ...
    def export_lanelet_network(
        ...
        for parametric_lane in self._planes:
            ...
            encoded_id = str(parametric_lane.id_).split(".")
            if len(encoded_id) >= 3:
                lanelet.opendrive_road_id = encoded_id[0]
                lanelet.opendrive_lane_section_id = encoded_id[1]
                lanelet.opendrive_lane_id = encoded_id[2]
                lanelet.opendrive_parametric_lane_id = str(parametric_lane.id_)
                lanelet.opendrive_lane_locations = {
                    (encoded_id[0], encoded_id[2])
                }
                lanelet.opendrive_lane_locations_detailed = {
                    (encoded_id[0], encoded_id[1], encoded_id[2])
                }
    ...
    def export_commonroad_scenario(
        ...
        location_kwargs = {}
        if (
            self._geo_ref is not None
            and self._geo_ref.strip().upper() != "LOCAL_CARTESIAN"
            and self._config.proj_string_odr is not None
        ):
