...
from automap_converter.core.config.coordinate_defaults import LANELET2_DEFAULT_CRS
from automap_converter.core.config.lanelet2_config import Lanelet2Config, lanelet2_config
from automap_converter.conversion.commonroad_to_lanelet2.utils import generate_unique_id
...
def _extract_and_convert_subtype_name(
    ...
    if "border" in common_subtypes:
        return "border", True
    if "shoulder" in common_subtypes:
...
class CR2LaneletConverter:
    ...
    def _create_transformer(self, scenario: Scenario):
        ...
        crs_to = CRS(LANELET2_DEFAULT_CRS)
    ...
    def __call__(self, scenario):
        ...
        self.left_ways = {}
        self.right_ways = {}
        self._odr_lane_to_osm_relation = {}
        self._odr_lane_to_cr_lanelet = {}
        self._traffic_sign_to_osm_way = {}
        self._pending_speed_sign_relations = []
        self._speed_sign_to_regulatory_element = {}

        # set origin shift according to translation in scenario
        if self.scenario_translation[0] != 0 or self.scenario_translation[1] != 0:
        ...
        self._add_regulatory_elements_for_speed_signs()
    ...
    def _add_regulatory_element_for_traffic_lights(self):
        ...
        for idx, ll in enumerate(self.lanelet_network.lanelets):
            ...
            traffic_light_reference_set = set()
                ...
                for way in self.osm.ways:
                    if self.osm.find_way_by_id(way).tag_dict.get("type") == "traffic_light":
                        ...
                        if str(lon_sign) == n_lon and str(lat_sign) == n_lat:
                            ...
                            if way not in traffic_light_reference_set:
                                traffic_light_reference_set.add(way)
            ...
            if len(ll.traffic_lights) > 0:
                ...
                if regulatory_element_id not in reg_elements_arr:
    ...
    def _convert_area(self, area: Area):
        ...
        tag_dict = {}
        if area.area_types and "subtype" not in tag_dict:
            for area_type in area.area_types:
                tag_dict["subtype"] = str(
    ...
    def _add_right_of_way_relation(self):
                            ...
                            for way_rel in self.osm.way_relations:
                                if (
                                    ...
                                    (
                                        ...
                                        self._append_from_sign(
                                            ll,
                                            way,
                                            self.osm.find_way_rel_by_id(way_rel),
                                            dict_stop_lines,
                                        )
                                    )
                                    append_unique(refers, sign_refers)
        ...
        if len(refers) > 0:
            ...
            for relation_id in dict.fromkeys(yield_ways + right_of_ways):
                way_relation = self.osm.find_way_rel_by_id(relation_id)
                if way_relation is not None and regulatory_id not in way_relation.regulatory_elements:
                    way_relation.regulatory_elements.append(regulatory_id)
    ...
    def _append_from_sign(
        ...
        if sign_name in ("YIELD", "STOP"):
            ...
            if stop_line_way is not None:
                ref_line.append(str(stop_line_way))
            else:
                logging.info("cr2lanelet::_append_from_sign: lanelet with yield/stop sign has no stop line")
        elif sign_name in ("RIGHT_OF_WAY", "PRIORITY"):
            right_of_ways.append(way_rel.id_)
    ...
    def _convert_traffic_sign(self, sign: TrafficSign):
        ...
        self.osm.add_way(
            Way(traffic_sign_wayid, [id1, id2], tag_dict=tag_dict)
        )
        self._traffic_sign_to_osm_way[str(sign.traffic_sign_id)] = str(traffic_sign_wayid)
    ...
    def _convert_lanelet(self, lanelet: Lanelet):
        ...
        self._append_opendrive_source_tags(lanelet, way_rel)

        # add the way relation to the osm
        self.osm.add_way_relation(way_rel)
        locations = set(getattr(lanelet, "opendrive_lane_locations", set()))
        road_id = getattr(lanelet, "opendrive_road_id", None)
        lane_id = getattr(lanelet, "opendrive_lane_id", None)
        if road_id is not None and lane_id is not None:
            locations.add((str(road_id), str(lane_id)))
        for road_id, lane_id in locations:
            key = (str(road_id), str(lane_id))
            self._odr_lane_to_osm_relation[key] = str(way_rel.id_)
            self._odr_lane_to_cr_lanelet[key] = lanelet.lanelet_id

    @staticmethod
    def _append_opendrive_source_tags(lanelet: Lanelet, way_rel: WayRelation) -> None:
        """Preserve OpenDRIVE road/lane provenance on generated Lanelet2 lanelets."""
        road_id = getattr(lanelet, "opendrive_road_id", None)
        lane_id = getattr(lanelet, "opendrive_lane_id", None)
        lane_section_id = getattr(lanelet, "opendrive_lane_section_id", None)
        parametric_lane_id = getattr(lanelet, "opendrive_parametric_lane_id", None)
        locations = sorted(
            (str(road), str(lane))
            for road, lane in set(getattr(lanelet, "opendrive_lane_locations", set()))
        )
        detailed_locations = sorted(
            (str(road), str(section), str(lane))
            for road, section, lane in set(
                getattr(lanelet, "opendrive_lane_locations_detailed", set())
            )
        )

        if road_id is not None:
            way_rel.tag_dict["source:opendrive:road"] = str(road_id)
        if lane_id is not None:
            way_rel.tag_dict["source:opendrive:lane"] = str(lane_id)
        if lane_section_id is not None:
            way_rel.tag_dict["source:opendrive:lane_section"] = str(lane_section_id)
        if parametric_lane_id is not None:
            way_rel.tag_dict["source:opendrive:parametric_lane"] = str(parametric_lane_id)
        if locations:
            way_rel.tag_dict["source:opendrive:lane_locations"] = ";".join(
                f"{road},{lane}" for road, lane in locations
            )
        if detailed_locations:
            way_rel.tag_dict["source:opendrive:lane_locations_detailed"] = ";".join(
                f"{road},{section},{lane}" for road, section, lane in detailed_locations
            )
        if road_id is not None or locations:
            way_rel.tag_dict["source:opendrive:mapping_status"] = "direct_converter_metadata"
    ...
    def _convert_speed_sign(self, lanelet: Lanelet, way_rel: WayRelation):
        """
        Register speed signs(CR) which will become regulatory elements(L2).

        Speed-limit regulatory elements refer to traffic_sign ways in Lanelet2.
        Traffic sign ways are generated after lanelet relations, so the actual
        regulatory relation is finalized in _add_regulatory_elements_for_speed_signs.
        ...
        # if so, remember it until the corresponding traffic_sign way exists.
            ...
            for sign in self.lanelet_network.traffic_signs:
                if sign.traffic_sign_id == traffic_sign:
                    if (
                        ...
                        self._pending_speed_sign_relations.append(
                            (str(sign.traffic_sign_id), str(way_rel.id_))
                        )

    def _add_regulatory_elements_for_speed_signs(self):
        """
        Create Lanelet2 speed_limit relations after all traffic_sign ways exist.
        """
        for traffic_sign_id, way_relation_id in self._pending_speed_sign_relations:
            sign = self.lanelet_network.find_traffic_sign_by_id(int(traffic_sign_id))
            if sign is None or len(sign.traffic_sign_elements) == 0:
                continue

            sign_element = sign.traffic_sign_elements[0]
            if sign_element.traffic_sign_element_id.name != "MAX_SPEED":
                continue

            speed_sign_id = self._speed_sign_to_regulatory_element.get(traffic_sign_id)
            if speed_sign_id is None:
                speed_sign_id = str(self.id_count)
                max_speed = (
                    sign_element.additional_values[0]
                    if sign_element.additional_values
                    else ""
                )
                refers = []
                traffic_sign_way_id = self._traffic_sign_to_osm_way.get(traffic_sign_id)
                if traffic_sign_way_id is not None:
                    refers.append(traffic_sign_way_id)
                self.osm.add_regulatory_element(
                    RegulatoryElement(
                        speed_sign_id,
                        refers=refers,
                ...
                self._speed_sign_to_regulatory_element[traffic_sign_id] = speed_sign_id

            way_rel = self.osm.find_way_rel_by_id(way_relation_id)
            if way_rel is not None and speed_sign_id not in way_rel.regulatory_elements:
                way_rel.regulatory_elements.append(speed_sign_id)
...
def convert(
    ...
    *,
    ...
    """Serialize one CommonRoad scenario as a Lanelet2 OSM document."""

    target_path = Path(output_file)
    target_path.parent.mkdir(parents=True, exist_ok=True)
    osm_root = CR2LaneletConverter(config=lanelet_conf, cr_config=general_conf)(scenario)
    target_path.write_bytes(
        etree.tostring(osm_root, xml_declaration=True, encoding="UTF-8", pretty_print=True)
    )
    return target_path
